#include "data structure/lazy_propagation_segment_tree.h"
#include "data structure/segment_tree.h"
#include "data structure/union-find.h"
#include "etc/fast_specific_digit_count_in_integer(0_to_N).h"
#include "geometry/convex_hull.h"
#include "graph/bellman-ford.h"
#include "graph/Bipartite graph matching (using Ford-Fulkerson's algorithm).h"
#include "graph/bipartite_graph_tester.h"
#include "graph/dijkstra.h"
#include "graph/Edmonds–Karp algorithm (maximum flow).h"
#include "graph/floyd-warshall.h"
#include "graph/Kosaraju's_algorithm (Strongly Connected Component).h"
#include "graph/prim.h"
#include "graph/SPFA (Shortest Path Faster Algorithm).h"
#include "graph/Tarjan's_algorithm (Strongly Connected Component).h"
#include "math/extended_euclidean.h"
#include "math/fast_binomial_coefficient.h"
#include "math/maximum_subarray_sum.h"
#include "string/aho-corasick.h"
#include "string/boyer-moore.h"
#include "string/KMP.h"
#include "string/Manacher's_algorith (all palindrome).h"
#include "string/online_palindrome_check.h"
#include "string/suffix_array_lcp.h"
#include <string>
#include <iostream>
#include <cstdio>

using namespace std;


void primTest() {
	size_t n, t;
	cin >> n >> t;
	
	prim::G.resize(n);
	for (size_t i = 0; i < t; i++) {
		int a, b, c;
		cin >> a >> b >> c;
		
		prim::G[a].emplace_back(c, b);
	}
	
	auto re = prim::prim();
	
	for (auto i = 0; i < re.size(); i++) {
		for (auto &p : re[i]) {
			printf("(%d -> %d) ", i, p.second);
		}
	}
}

void bellmanTest() {
	size_t n, t;
	cin >> n >> t;
	
	BellmanFord::G.resize(n);
	for (size_t i = 0; i < t; i++) {
		int a, b, c;
		cin >> a >> b >> c;
		
		BellmanFord::G[a].emplace_back(c, b);
	}
	
	auto re = BellmanFord::bellman_ford(0);
	
	for (auto d : re) {
		printf("%d ", d);
	}
	
	printf("\n%d", BellmanFord::hasNegativCycle() ? 1 : 0);
}

void boyerMooreTest() {
	string t, p;
	
	cin >> t >> p;
	
	printf("%zd\n", BoyerMoore::boyer_moore(t, p));
	
	for (auto &c : BoyerMoore::boyer_moore_all(t, p)) {
		printf("%zd ", c);
	}
}

void KMPTest() {
	string t, p;
	
	cin >> t >> p;
	
	printf("%zd", KMP::KMP(t, p));
	
	puts("");
	for (auto t : KMP::failure) {
		printf("%zd ", t);
	}
}

void ahoCorasickTest() {
	int n;
	cin >> n;
	
	string str;
	for (int i = 0; i < n; i++) {
		cin >> str;
		AhoCorasick::patterns.emplace_back(str);
	}
	
	cin >> AhoCorasick::target;
	
	AhoCorasick::build();
	
	for (const auto t : AhoCorasick::search()) {
		cout << "at " << t.first << ", pattern \"" << AhoCorasick::patterns[t.second] << "\" found\n";
	}
}

void suffixArrayTest() {
	string t;
	getline(cin, t);
	
	for (size_t p : SuffixArray_LCP::suffixArray(t)) {
		cout << t.substr(p) << '\n';
	}
}

void segmentTreeTest() {
	size_t n, t;
	cin >> t;
	vector<size_t> arr;
	
	for (size_t i = 0; i < t; i++) {
		cin >> n;
		arr.emplace_back(n);
	}
	
	LazySegmentTree<size_t> tree(arr);
	
	cout << tree.query(1, 5);
}

void extended_euclidean_test() {
	using namespace ExtendedEuclid;
	
	int a, b;
	scanf("%d %d", &a, &b);
	
	int re = extended_euclid(a, b);
	printf("%d %d %d\n", m, n, re);
}

void online_palindrome_check() {
	string s;
	cin >> s;
	OnlinePalindromeChecker checker;
	for (char c : s) {
		printf("%c : %d\n", c, checker.insertChar(c - 'a'));
	}
}

void kosaraju_test() {
	size_t n, m, a, b;
	cin >> n >> m;
	
	vector<vector<size_t>> map(n), revMap(n);
	
	while (m--) {
		cin >> a >> b;
		a--, b--;
		map[a].emplace_back(b);
		revMap[b].emplace_back(a);
	}
	
	const auto &arr = SCC::SCC(map, revMap);
	for (auto v : arr) printf("%zu ", v);
	puts("");
}

void targan_test() {
	int n, m, a, b;
	
	auto &arr = SCC::adj;
	cin >> n >> m;
	arr.resize(n);
	
	while (m--) {
		cin >> a >> b;
		a--, b--;
		arr[a].emplace_back(b);
	}
	
	const auto &ret = SCC::tarjanSCC();
	for (auto v : ret) printf("%d ", v);
}

int main() {
	segmentTreeTest();
}